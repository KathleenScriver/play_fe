...
const getPlaylistSelectOptions = () => {
  fetch(`${playBackUrl}/playlists`)
    .then(response => response.json())
    .then(allPlaylistsInfo => setPlaylistSelectOptions(allPlaylistsInfo))
    .catch(error => ({ error }));
};

const setPlaylistSelectOptions = (playlists) => {
...
const displayPlaylistOptions = (event) => {
  ...
  $('.playlist-drop-down li').click((e) => {
    e.stopPropagation();
    let playlistId = $(e.target).attr('value');
    let songId = $(e.target).parents('tr').attr('value');
